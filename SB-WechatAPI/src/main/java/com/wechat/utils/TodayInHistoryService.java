...
public class TodayInHistoryService {
    ...
    private static String extract2(String html) {
        Elements elements = Jsoup.parse(html).select("div.listren > ul > li > a");
        StringBuffer sb = new StringBuffer();
        for (Element element : elements) {
            sb.append(element.text().replace("（图）", "").replace("&nbsp;", "")).append("\n");
        }
